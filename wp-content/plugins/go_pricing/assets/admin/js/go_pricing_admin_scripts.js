	...
	/ ---------------------------------------------------------------------- */
		...
		function seachTable($elem) {
			...
			$goPricingAdmin.find('#go-pricing-table-manager').find('.gwa-thumb').not('.gwa-thumb-new').each(function(index, element) {
				if ($(element).find('.gwa-thumb-title').text().toLowerCase().indexOf($input.val().toLowerCase()) == -1 && $(element).find('.gwa-thumb-title').data('id').indexOf($input.val().toLowerCase()) == -1) {
					$(element).hide();
				} else {
					$(element).show();
					thumbCnt++;					
				...
				file_frame.on('select', function() {
					...
					switch( file_frame.state().get('selection').first().toJSON().type ) {
						...
						case 'image' :
							...
							if (optsHtml != '') $('<select>', { html : optsHtml, style : 'margin-bottom:20px;' }).appendTo($media);
